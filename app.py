...
def init_app():
    """Inicializácia DB a modelu."""
    global pipeline
    tmp_conn = init_db()
    populate_sample_data(tmp_conn)
    pipeline = load_model()
    if pipeline is None:
        log.info("Trénujem model na základných dátach...")
...
# ── Inicializácia pri štarte (funguje aj s gunicorn) ──────────────────────────
...
if __name__ == "__main__":
    ...
    log.info("Server štartuje na porte %d", port)
